...
Ключевая граница уже знакома по предыдущим карточкам. Vue создаёт отслеживаемые
reactive cells/Proxy: чтение связывает потребителя с зависимостью, а запись уведомляет
его об изменении. React не отслеживает произвольные чтения и записи JavaScript.
Компонент получает новый render snapshot после явного update его state/props/Context
или уведомления подписанного store. Поэтому выбирать нужно сначала владельца и
причину обновления, а уже затем API.
...
Семантическая часть выполняется отдельно. Команда ниже только собирает review-пакет и сама
агента не запускает:

    pnpm session:review

После сбора пакета отдельно попросите Codex проверить качество объяснений по rubric. Автоматический quiz не способен
определить, действительно ли reason объясняет ownership или просто содержит случайный текст.
